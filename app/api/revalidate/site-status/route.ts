...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const { allowed } = checkRateLimit(`revalidate-site-status:${clientIp}`, RATE_LIMIT, RATE_LIMIT_WINDOW_MS);
  ...
  // Triggered by the backend's revalidate_order_halt provider method the
  // moment an admin flips the kill switch — needs the tag to expire
  // immediately, not the stale-while-revalidate default.
  revalidateTag("site-status", { expire: 0 });
